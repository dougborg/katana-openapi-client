...
This guide is intentionally summary-only — each pattern points to the canonical ADR that
owns it. Don't enumerate ADRs by name and number here; link to the index README files
instead and let those stay current. (See "ADR Indexes" at the end.)

## Project Structure

This is a **monorepo with three packages** (Python workspace + a sibling npm package):

```
katana-openapi-client/                 # Monorepo root
├── katana_public_api_client/          # Python client library (PyPI: katana-openapi-client)
│   └── docs/adr/                      # Client-specific ADRs
├── katana_mcp_server/                 # MCP server for AI agents (PyPI: katana-mcp-server)
│   └── docs/adr/                      # MCP-specific ADRs
├── packages/katana-client/            # TypeScript client (npm: @katana-mrp/client)
└── docs/adr/                          # Shared/monorepo ADRs
```

Detailed file-by-file rules live in [FILE_ORGANIZATION.md](FILE_ORGANIZATION.md).
...
**ADR:** ADR-001 in the
[client ADR index](../../../../katana_public_api_client/docs/adr/README.md).
...
For the full pattern table (when to use each helper, anti-patterns to avoid), see
[CLAUDE.md "API Response Handling Best Practices"](../../../../CLAUDE.md) and the
...
**ADR:** ADR-007 (see also ADR-008 for "Avoid Builder Pattern").
...
**ADRs:** ADR-010 (initial design) plus the MCP-server ADRs in the
[MCP ADR index](../../../../katana_mcp_server/docs/adr/README.md) — including the
typed-cache and tool-interface ADRs that have shaped the surface since.
...
**ADR:** ADR-009 (in the shared/monorepo ADR index).
...
Don't enumerate ADRs here — the per-scope ADR README files own the canonical list and
stay current as new ADRs land. Use these:
