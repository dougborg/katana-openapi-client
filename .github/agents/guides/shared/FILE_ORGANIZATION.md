...
The high-level "EDITABLE vs GENERATED" rule is summarized in
[CLAUDE.md "File Rules"](../../../../CLAUDE.md). This guide goes one level deeper —
*which* directories are generated, *what* the regen process does, and *how* the import
paths are organized — so you don't accidentally edit generated code or import from a
path that disappears on regeneration.
...
Don't write `if response.status_code == 200`. Use `unwrap_as`, `unwrap_data`, or
`is_success` from `katana_public_api_client.utils`. The full anti-pattern table is
covered in CLAUDE.md and the
