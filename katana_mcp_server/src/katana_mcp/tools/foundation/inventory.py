...
async def list_stock_adjustments(
    ...
    For batch lookup by known IDs, pass `ids=[...]` and get the matching
    adjustments back in a single call. Use for discovery — find recent
    adjustments at a location, adjustments touching a specific variant, or
    adjustments matching a reason substring. Returns a JSON list response
    with one summary entry per adjustment (number, location, dates, reason,
    row count). Set `include_rows=true` to also populate per-row details
    (variant_id, quantity, cost_per_unit).
