...
class TestCriticalAPIGaps:
    ...
    def test_gap_analysis_shows_acceptable_coverage(self, gap_analysis: dict[str, Any]):
        ...
        if len(critical_missing) > 5:
            critical_issues.append(
                f"Too many critical endpoints missing: {len(critical_missing)}"
            )
        ...
        if len(crud_issues) > 3:
            critical_issues.append(
                f"Too many CRUD completeness issues: {len(crud_issues)}"
            )

        if critical_issues:
            pytest.fail(
                "API coverage has critical issues:\n"
    ...
    def test_documentation_validation_files_exist(self):
        ...
        assert "endpoints" in validation_data, (
            "Validation results should contain endpoint analysis"
        )
        assert "schemas" in validation_data, (
            "Validation results should contain schema analysis"
        )
