...
class TestComprehensiveAPIValidation:
    ...
    def validation_results(
        self, validator: APIDocumentationValidator
    ) -> dict[str, Any]:
            ...
            if "/{id}" in path and missing_methods:
                ...
                if critical_missing:
                    critical_method_mismatches.append(
                        {"path": path, "missing_critical_methods": critical_missing}
                    )
    ...
    def test_schema_definition_completeness(self, validation_results: dict[str, Any]):
        ...
        critical_schemas_without_descriptions = [
            schema
            for schema in missing_descriptions
            if schema["schema"]
            in {
                "Product",
                "Customer",
    ...
    def test_openapi_specification_version_compatibility(
    ...
    def test_validation_summary_within_acceptable_limits(
        ...
        if failures:
            pytest.fail(
                "Validation metrics exceed acceptable thresholds:\n"
