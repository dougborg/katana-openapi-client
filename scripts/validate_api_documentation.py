...
class APIDocumentationValidator:
    ...
    def __init__(self, repo_root: Path):
        ...
        self.validation_results = {
            ...
            "documentation": {"missing_files": [], "content_mismatches": []},
            "summary": {},
        }
    ...
    def validate_endpoints(self) -> None:
        ...
        self.validation_results["endpoints"]["missing_in_current"] = list(
            comp_paths - current_paths
        )
        self.validation_results["endpoints"]["missing_in_comprehensive"] = list(
            current_paths - comp_paths
        )
                ...
                self.validation_results["endpoints"]["parameter_mismatches"].append(
                    {
                        ...
                        "missing_in_current": list(
                            comp_param_names - current_param_names
                        ),
                        "extra_in_current": list(
                            current_param_names - comp_param_names
                        ),
                    }
                )
    ...
    def validate_schemas(self) -> None:
        ...
        self.current_spec.get("components", {}).get("schemas", {})
        self.comprehensive_spec.get("components", {}).get("schemas", {})
        ...
        for doc_file in doc_files:
            try:
                with open(doc_file, encoding="utf-8") as f:
                    ...
                    referenced_schemas.update(
                        schema.strip('"') for schema in schemas_in_content
                    )
            except Exception:
                continue

        current_schemas = set(
            self.current_spec.get("components", {}).get("schemas", {}).keys()
        )
    ...
    def _validate_schema_completeness(self) -> None:
        ...
        for schema_name, schema_def in current_schemas.items():
            ...
            if issues:
                incomplete_schemas.append({"schema": schema_name, "issues": issues})
        ...
        for _path, methods in paths.items():
            for method in methods:
                ...
                if operation_id:
                    ...
                    filename = (
                        re.sub(r"([A-Z])", r"_\1", operation_id).lower().lstrip("_")
                    )
    ...
    def generate_summary(self) -> None:
        ...
        self.validation_results["summary"] = {
            ...
            "total_endpoints_comprehensive": len(
                self.comprehensive_spec.get("paths", {})
            ),
            "total_schemas_current": len(
                self.current_spec.get("components", {}).get("schemas", {})
            ),
            "total_schemas_comprehensive": len(
                self.comprehensive_spec.get("components", {}).get("schemas", {})
            ),
            "endpoints_missing_in_current": len(endpoints["missing_in_current"]),
            "endpoints_missing_in_comprehensive": len(
                endpoints["missing_in_comprehensive"]
            ),
            ...
            "schema_issues": len(schemas["missing_definitions"])
            + len(schemas["property_mismatches"]),
    ...
    def print_validation_report(self) -> None:
        ...
        print(
            f"Comprehensive docs endpoints: {summary['total_endpoints_comprehensive']}"
        )
        ...
        print(
            f"Missing in comprehensive docs: {summary['endpoints_missing_in_comprehensive']}"
        )
        ...
        if results["endpoints"]["missing_in_current"]:
            ...
            if len(results["endpoints"]["missing_in_current"]) > 10:
                print(
                    f"  ... and {len(results['endpoints']['missing_in_current']) - 10} more"
                )
        ...
        if results["endpoints"]["method_mismatches"]:
            ...
            for mismatch in results["endpoints"]["method_mismatches"][:5]:
                ...
                if mismatch["missing_in_current"]:
                    print(
                        f"    Missing methods: {', '.join(mismatch['missing_in_current'])}"
                    )
                if mismatch["extra_in_current"]:
                    print(
                        f"    Extra methods: {', '.join(mismatch['extra_in_current'])}"
                    )
