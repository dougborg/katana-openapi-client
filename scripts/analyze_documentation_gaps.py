...
class DocumentationGapAnalyzer:
    ...
    def analyze_missing_endpoints(self) -> Dict[str, Any]:
        ...
        for endpoint in missing_endpoints:
            if any(pattern.replace("{id}", "").rstrip("/") in endpoint for pattern in critical_patterns):
                categorized["critical"].append(endpoint)
            elif any(pattern.replace("{id}", "").rstrip("/") in endpoint for pattern in important_patterns):
    ...
    def analyze_method_gaps(self) -> Dict[str, Any]:
        ...
        for mismatch in method_mismatches:
            ...
            if "/{id}" in path:
                crud_missing = [m for m in missing_methods if m in {"get", "patch", "delete"}]
                if crud_missing:
    ...
    def analyze_parameter_gaps(self) -> Dict[str, Any]:
        ...
        parameter_mismatches = self.validation_results["endpoints"]["parameter_mismatches"]
        ...
        for mismatch in parameter_mismatches:
            ...
            filtering_params = [p for p in missing_params if p in {
            ...
            # Other parameters
            other_params = [p for p in missing_params if p not in {
    ...
    def analyze_schema_gaps(self) -> Dict[str, Any]:
        ...
        for schema_issue in schema_issues:
            ...
            property_issues = [issue for issue in issues if "property_" in issue and "missing_description" in issue]
            if property_issues:
                analysis["properties_missing_descriptions"].append({
                    "schema": schema_name,
                    "properties_without_descriptions": [
                        issue.replace("property_", "").replace("_missing_description", "") 
    ...
    def generate_implementation_recommendations(self) -> Dict[str, Any]:
        ...
        if method_analysis["crud_completeness_issues"]:
            recommendations["immediate_actions"].append({
                ...
                "details": [issue["path"] for issue in method_analysis["crud_completeness_issues"][:3]],
                "effort": "Medium",
                "impact": "High"
            })
                ...
                specifications["missing_endpoint_specs"].append({
                    ...
                    "methods": {
                        "get": {
                            ...
                            "responses": {"200": {"description": "Customer details", "schema": "$ref:Customer"}}
                        },
                        "patch": {
                            "summary": "Update a customer",
                            "parameters": [{"name": "id", "in": "path", "required": True, "schema": {"type": "integer"}}],
                            "requestBody": {"schema": "$ref:CustomerUpdate"},
                            "responses": {"200": {"description": "Updated customer", "schema": "$ref:Customer"}}
                        },
                        "delete": {
                            "summary": "Delete a customer", 
                            "parameters": [{"name": "id", "in": "path", "required": True, "schema": {"type": "integer"}}],
                            "responses": {"204": {"description": "Customer deleted successfully"}}
                        }
                    }
                })
    ...
    def print_comprehensive_analysis(self) -> None:
        ...
        print(f"CRUD completeness issues: {len(method_analysis['crud_completeness_issues'])}")
        if method_analysis["crud_completeness_issues"]:
            for issue in method_analysis["crud_completeness_issues"][:3]:
        ...
        print(f"Endpoints missing pagination: {len(parameter_analysis['pagination_missing'])}")
        print(f"Endpoints missing filtering: {len(parameter_analysis['filtering_missing'])}")
        
        print(f"\n📋 SCHEMA ISSUES")
        print(f"Critical schemas without descriptions: {len(schema_analysis['critical_schemas_without_descriptions'])}")
        if schema_analysis["critical_schemas_without_descriptions"]:
            print(f"  - {', '.join(schema_analysis['critical_schemas_without_descriptions'])}")
