...
| **Spec-drift override registry**                | `audit-spec-strict` CI gate, `docs/upstream-specs/audit-overrides.yaml` (4 categories: intentional / `local_correct_upstream_wrong` punch-list / `both_wrong_live_correct` / `upstream_correct_local_wrong` needs `fix_tracked_in`), narrow `(endpoint, kind, field, values)` matching, stale-override detection, `--no-overrides` for raw drift — every finding terminates in a fix or a registry entry | [docs/upstream-specs/README.md](docs/upstream-specs/README.md)                                                 |
