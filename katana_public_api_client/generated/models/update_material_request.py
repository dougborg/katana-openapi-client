...
class UpdateMaterialRequest(BaseModel):
    ...
    purchase_uom: Optional[constr(strict=True, max_length=7)] = Field(default=None, description="If used, then purchase_uom_conversion_rate must have a value as well.")
    purchase_uom_conversion_rate: Optional[Union[confloat(le=1000000000000, strict=True), conint(le=2147483647, strict=True)]] = Field(default=None, description="If used, then purchase_uom must have a value as well.")
    configs: Optional[conlist(UpdateMaterialRequestConfigsInner, min_items=1)] = Field(default=None, description="When updating configs, all configs and values must be provided. Existing ones are matched,         new ones are created, and configs not provided in the update are deleted.")
    custom_field_collection_id: Optional[conint(strict=True, le=2147483647)] = None
    additional_properties: Dict[str, Any] = {}
    __properties = ["name", "uom", "category_name", "default_supplier_id", "additional_info", "batch_tracked", "is_sellable", "is_archived", "purchase_uom", "purchase_uom_conversion_rate", "configs", "custom_field_collection_id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateMaterialRequest:
        """Create an instance of UpdateMaterialRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateMaterialRequest:
        """Create an instance of UpdateMaterialRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpdateMaterialRequest.parse_obj(obj)

        _obj = UpdateMaterialRequest.parse_obj({
            "name": obj.get("name"),
            "uom": obj.get("uom"),
            ...
            "configs": [UpdateMaterialRequestConfigsInner.from_dict(_item) for _item in obj.get("configs")] if obj.get("configs") is not None else None,
