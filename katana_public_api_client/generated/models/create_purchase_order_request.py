...
class CreatePurchaseOrderRequest(BaseModel):
    ...
    currency: Optional[StrictStr] = Field(default=None, description="Active ISO 4217 currency code (e.g. USD, EUR).")
    ...
    tracking_location_id: Optional[conint(strict=True, le=2147483647)] = Field(default=None, description="Submittable only when entity_type is outsourced")
    additional_info: Optional[StrictStr] = None
    purchase_order_rows: conlist(PurchaseOrderRowRequest, min_items=1) = Field(...)
    __properties = ["order_no", "entity_type", "supplier_id", "currency", "status", "expected_arrival_date", "order_created_date", "location_id", "tracking_location_id", "additional_info", "purchase_order_rows"]
    ...
    def entity_type_validate_enum(cls, value):
        ...
        if value not in ('regular', 'outsourced',):
    ...
    def from_dict(cls, obj: dict) -> CreatePurchaseOrderRequest:
        ...
        _obj = CreatePurchaseOrderRequest.parse_obj({
            ...
            "purchase_order_rows": [PurchaseOrderRowRequest.from_dict(_item) for _item in obj.get("purchase_order_rows")] if obj.get("purchase_order_rows") is not None else None
        })
        return _obj
