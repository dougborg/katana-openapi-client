...
class CreateSalesReturnRequest(BaseModel):
    ...
    """
    customer_id: StrictInt = Field(default=..., description="ID of the customer initiating the return")
    sales_order_id: Optional[StrictInt] = Field(default=None, description="ID of the original sales order being returned")
    order_no: StrictStr = Field(default=..., description="Return order reference number")
    return_location_id: StrictInt = Field(default=..., description="ID of the location where items are being returned to")
    currency: Optional[StrictStr] = Field(default=None, description="Currency code (e.g., USD, EUR)")
    order_created_date: Optional[datetime] = Field(default=None, description="Date when the original order was created")
    additional_info: Optional[StrictStr] = Field(default=None, description="Optional notes or comments about the return")
    sales_return_rows: conlist(CreateSalesReturnRowRequest) = Field(default=..., description="Array of items being returned")
    __properties = ["customer_id", "sales_order_id", "order_no", "return_location_id", "currency", "order_created_date", "additional_info", "sales_return_rows"]
    ...
    def from_dict(cls, obj: dict) -> CreateSalesReturnRequest:
        ...
        _obj = CreateSalesReturnRequest.parse_obj({
            ...
            "sales_return_rows": [CreateSalesReturnRowRequest.from_dict(_item) for _item in obj.get("sales_return_rows")] if obj.get("sales_return_rows") is not None else None
        })
        return _obj
