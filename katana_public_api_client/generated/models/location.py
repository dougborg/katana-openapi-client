...
class Location(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, LocationAnyOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `LocationAnyOf`")
        else:
            return v

        # validate data type: DeletableEntity
        if not isinstance(v, DeletableEntity):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeletableEntity`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Location with anyOf schemas: DeletableEntity, LocationAnyOf. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Location:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Location with anyOf schemas: DeletableEntity, LocationAnyOf. Details: " + ", ".join(error_messages))
        else:
            return instance
