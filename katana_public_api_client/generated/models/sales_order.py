...
class SalesOrder(BaseModel):
    ...
    sales_order_rows: Optional[conlist(SalesOrderRow)] = None
    ...
    production_status: Optional[StrictStr] = None
    tracking_number: Optional[StrictStr] = None
    tracking_number_url: Optional[StrictStr] = None
    billing_address_id: Optional[StrictInt] = None
    shipping_address_id: Optional[StrictInt] = None
    addresses: Optional[conlist(SalesOrderAddress)] = None
    __properties = ["created_at", "updated_at", "id", "customer_id", "order_no", "source", "order_created_date", "delivery_date", "picked_date", "location_id", "status", "currency", "conversion_rate", "conversion_date", "invoicing_status", "total", "total_in_base_currency", "additional_info", "customer_ref", "sales_order_rows", "ecommerce_order_type", "ecommerce_store_name", "ecommerce_order_id", "product_availability", "product_expected_date", "ingredient_availability", "ingredient_expected_date", "production_status", "tracking_number", "tracking_number_url", "billing_address_id", "shipping_address_id", "addresses"]

    @validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('NOT_SHIPPED', 'PARTIALLY_PACKED', 'PARTIALLY_DELIVERED', 'PACKED', 'DELIVERED', 'PENDING',):
            raise ValueError("must be one of enum values ('NOT_SHIPPED', 'PARTIALLY_PACKED', 'PARTIALLY_DELIVERED', 'PACKED', 'DELIVERED', 'PENDING')")
        return value
    ...
    def ingredient_availability_validate_enum(cls, value):
        ...
        if value not in ('PROCESSED', 'IN_STOCK', 'NOT_AVAILABLE', 'EXPECTED', 'NO_RECIPE', 'NOT_APPLICABLE',):
            raise ValueError("must be one of enum values ('PROCESSED', 'IN_STOCK', 'NOT_AVAILABLE', 'EXPECTED', 'NO_RECIPE', 'NOT_APPLICABLE')")
        return value
    ...
    def production_status_validate_enum(cls, value):
        ...
        if value not in ('NOT_STARTED', 'NONE', 'NOT_APPLICABLE', 'IN_PROGRESS', 'BLOCKED', 'DONE',):
            raise ValueError("must be one of enum values ('NOT_STARTED', 'NONE', 'NOT_APPLICABLE', 'IN_PROGRESS', 'BLOCKED', 'DONE')")
        return value
    ...
    def to_dict(self):
        ...
        # and __fields_set__ contains the field
        if self.production_status is None and "production_status" in self.__fields_set__:
        ...
        # and __fields_set__ contains the field
        if self.billing_address_id is None and "billing_address_id" in self.__fields_set__:
            _dict['billing_address_id'] = None

        # set to None if shipping_address_id (nullable) is None
        # and __fields_set__ contains the field
        if self.shipping_address_id is None and "shipping_address_id" in self.__fields_set__:
    ...
    def from_dict(cls, obj: dict) -> SalesOrder:
        ...
        _obj = SalesOrder.parse_obj({
            ...
            "sales_order_rows": [SalesOrderRow.from_dict(_item) for _item in obj.get("sales_order_rows")] if obj.get("sales_order_rows") is not None else None,
