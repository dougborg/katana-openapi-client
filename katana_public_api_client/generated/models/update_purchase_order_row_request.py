...
class UpdatePurchaseOrderRowRequest(BaseModel):
    ...
    quantity: (
        confloat(le=100000000000000000, strict=True)
        | conint(le=2147483647, strict=True)
        | None
    ) = Field(default=None, description="Updatable only when received_date is null")
    variant_id: conint(strict=True, le=2147483647) | None = Field(
        default=None, description="Updatable only when received_date is null"
    )
    tax_rate_id: conint(strict=True, le=2147483647) | None = Field(
        default=None, description="Updatable only when received_date is null"
    )
    group_id: conint(strict=True, le=2147483647) | None = Field(
        default=None, description="Updatable only when received_date is null"
    )
    price_per_unit: (
        confloat(le=100000000000000000, ge=0, strict=True)
    ...
    received_date: StrictStr | None = Field(
        default=None, description="Updatable only when already set"
    )
    arrival_date: StrictStr | None = Field(
        default=None, description="Updatable only when received_date is not null"
    )
    __properties = [
        "quantity",
        "variant_id",
