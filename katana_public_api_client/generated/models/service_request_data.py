...
class ServiceRequestData(BaseModel):
    ...
    type: StrictStr = Field(
        default=..., description="Resource type must be 'services'."
    )
    attributes: ServiceInputAttributes = Field(...)
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ("services",):
            raise ValueError("must be one of enum values ('services')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ServiceRequestData:
        """Create an instance of ServiceRequestData from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ServiceRequestData:
        """Create an instance of ServiceRequestData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ServiceRequestData.parse_obj(obj)

        _obj = ServiceRequestData.parse_obj(
            {
                "type": obj.get("type"),
                "attributes": ServiceInputAttributes.from_dict(obj.get("attributes"))
